...
@router.get("")
def evaluation(merchant_id: str="m_demo", db: Session = Depends(get_db)):
    return full_evaluation(db, merchant_id)

@router.get("/replay")
def replay(merchant_id: str="m_demo", n: int=1000, db: Session = Depends(get_db)):
    return offline_replay(db, merchant_id, n)

@router.get("/kpis")
def kpis(merchant_id: str="m_demo", db: Session = Depends(get_db)):
