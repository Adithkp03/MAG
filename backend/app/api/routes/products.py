...
def list_products(merchant_id: str = None, q: str = "", category: str = "", max_price: int = None, limit: int = Query(50, ge=1, le=100), offset: int = Query(0, ge=0), db: Session = Depends(get_db)):
    ...
    items = [{"id":r.id,"name":r.name,"description":r.description,"price":r.price,"category":r.category,"stock":r.stock,"merchant_id":r.merchant_id} for r in paged]
...
def search(q: str = Query("", description="search query"), merchant_id: str = None, category: str = "", max_price: int = None, limit: int = Query(20, ge=1, le=50), offset: int = Query(0, ge=0), db: Session = Depends(get_db)):
    ...
    total = len(rows)
    paged = rows[offset:offset+limit]
    return {"products": [{"id":r.id,"name":r.name,"description":r.description,"price":r.price,"category":r.category,"stock":r.stock,"merchant_id":r.merchant_id} for r in paged], "total": total, "limit": limit, "offset": offset}
